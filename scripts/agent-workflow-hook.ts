...
// intent: DEC-004 (Workflow/docs-template-v1-migration) — Keep hook context bounded so guardrails do not replace task-specific judgement and QA.
...
// intent: DEC-004 (Workflow/docs-template-v1-migration) — Audit durable consequences without letting a guardrail authorize scope expansion.
...
// intent: DEC-006 (Workflow/docs-template-v1-2-migration) — Surface the document requirement while the implementation approach can still change, without settling Risk from the path alone.
...
// intent: DEC-006 (Workflow/docs-template-v1-2-migration) — apply_patch carries targets in the command, so write-time Risk notice must inspect both tool input shapes.
const writeTargets = (
...
// intent: DEC-006 (Workflow/docs-template-v1-2-migration) — Use one workflow-sensitive path predicate for write-time notice and working-tree closure evidence.
...
export const analyzeStop = (
  ...
  // intent: DEC-006 (Workflow/docs-template-v1-2-migration) — Final-message wording alone cannot prove workflow closure, so require working-tree intent or QA evidence.
  ...
  const actions = [
    ...
    // intent: DEC-006 (Workflow/docs-template-v1-2-migration) — Risk High needs the whole document chain, while the hook still leaves classification to the worker.
    ...(grouped.workflow.length > 0
      ? [
