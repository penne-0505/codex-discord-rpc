...
assert(
  ...
  "DEC-004 keep per-prompt audit short and evidence-based",
);
...
assert(
  ...
  "DEC-004 add durable write audit context",
);
...
assert(
  ...
  "DEC-004 avoid write audit noise on read-only tools",
);
...
assert(
  ...
  "DEC-006 warn about the Risk High document chain before workflow-sensitive writes",
);
...
assert(
  ...
  "DEC-006 write-time notice reports the requirement without settling Risk",
);

assert(
  writeAudit?.decision === "context" &&
    !writeAudit.context.includes("workflow-sensitive"),
  "DEC-006 keep ordinary source writes free of the workflow-sensitive notice",
);
...
assert(
  ...
  "DEC-006 detect workflow-sensitive apply_patch targets like Codex uses",
);
...
assert(
  ...
  "DEC-006 single workflow-sensitive predicate is shared by write and stop audits",
);
...
assert(
  ...
  "DEC-004 stop hook rejects verification without independent audit",
);
...
assert(
  ...
  "DEC-004 stop hook allows verification with multi-perspective audit",
);

assert(
  auditEvidenceCount("反証を確認し、影響範囲と長期保守性を監査した。") === 3,
  "DEC-004 count distinct audit perspectives",
);
...
assert(
  ...
  "DEC-006 stop hook requires closure from working-tree facts, not wording alone",
);
...
assert(
  ...
  "DEC-006 accompanying QA docs satisfy the working-tree evidence condition",
);
...
assert(
  ...
  "DEC-006 accompanying intent docs satisfy the working-tree evidence condition",
);
...
assert(
  ...
  "DEC-006 stop message spells out the Risk High document chain for workflow-sensitive paths",
);
...
assert(
  ...
  "DEC-006 keep the Risk High chain out of non workflow-sensitive closures",
);
