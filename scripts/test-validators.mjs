...
const FRONTMATTER_MARKER_FIXTURES = [
  {
    source: "_evals/validator-fixtures/frontmatter/known/intent-schema.md",
    target: "_docs/intent/Core/schema-marker/decision.md",
  },
  {
    source: "_evals/validator-fixtures/frontmatter/known/qa-schema.md",
    target: "_docs/qa/Core/schema-marker/test-plan.md",
  },
  {
    source: "_evals/validator-fixtures/frontmatter/unknown/future-schema.md",
    target: "_docs/guide/Core/schema-marker/usage.md",
  },
];
...
const runFrontmatterMarkerCase = async () => {
  const repoRoot = Deno.cwd();
  const temp = await Deno.makeTempDir({ prefix: "docs-frontmatter-markers-" });
  try {
    for (const fixture of FRONTMATTER_MARKER_FIXTURES) {
      const target = `${temp}/${fixture.target}`;
      await ensureDir(target.slice(0, target.lastIndexOf("/")));
      await write(target, await Deno.readTextFile(fixture.source));
    }
    ...
    return output.code === 0 &&
      ...
      stderr.includes("unknown field: future_schema");
  } finally {
...
ok = await (async () => {
  const passed = await runFrontmatterMarkerCase();
  if (passed) {
    console.log(
      "PASS frontmatter accepts known schema markers and warns on unknown markers",
    );
    return true;
  }
  console.error(
    "FAIL frontmatter marker compatibility or unknown-field warning",
  );
  return false;
})() && ok;
